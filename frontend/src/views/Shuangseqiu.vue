<template>
  <div class="shuangseqiu-container">
    ...
    <el-card class="table-card">
      <template #header>
        <div class="card-header">
          <span class="card-title">双色球开奖记录</span>
        ...
        <el-table-column label="红球" min-width="420">
          <template #default="{ row }">
            <div class="balls-container">
              <span class="ball red-ball">{{ row.red_ball_1 }}</span>
              <span class="ball red-ball">{{ row.red_ball_2 }}</span>
              <span class="ball red-ball">{{ row.red_ball_3 }}</span>
              <span class="ball red-ball">{{ row.red_ball_4 }}</span>
              <span class="ball red-ball">{{ row.red_ball_5 }}</span>
              <span class="ball red-ball">{{ row.red_ball_6 }}</span>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="蓝球" min-width="100">
          <template #default="{ row }">
            <span class="ball blue-ball">{{ row.blue_ball }}</span>
          </template>
        </el-table-column>
    ...
    >
      <el-tabs v-model="statsType" @tab-change="loadStatistics">
        <el-tab-pane label="红球统计" name="red">
          <div class="statistics-container">
            <div
              ...
              class="stat-item"
            >
              <div class="stat-number red-ball">{{ stat.number }}</div>
        ...
        <el-tab-pane label="蓝球统计" name="blue">
          <div class="statistics-container">
            <div
              v-for="stat in statistics"
              :key="stat.number"
              class="stat-item"
            >
              <div class="stat-number blue-ball">{{ stat.number }}</div>
              <div class="stat-count">出现 {{ stat.count }} 次</div>
              <el-progress
                :percentage="getPercentage(stat.count)"
                :color="'#409eff'"
...
import { shuangseqiuAPI, taskAPI } from '@/api/lottery'
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await shuangseqiuAPI.getList({
...
const handleFetch = async (issue) => {
  loading.value = true
  try {
    await shuangseqiuAPI.fetch(issue)
    // 成功或已存在的消息会由拦截器自动显示
    loadData()
  } catch (error) {
...
const loadStatistics = async () => {
  try {
    const res = await shuangseqiuAPI.getStatistics(statsType.value)
