    ...
    <el-card>
      <template #header>
        <div class="card-header">
          <span class="card-title">双色球号码推荐</span>
        </div>
      </template>
        ...
        <el-card shadow="never" class="select-card">
          ...
          <div class="ball-selection">
            <div class="ball-group">
              <div class="group-title">红球（选择6个）</div>
              <div class="ball-list">
                <span
                  v-for="num in 33"
                  :key="`red-${num}`"
                  class="ball-item red-ball-item"
                  :class="{ selected: selectedRed.includes(num) }"
            ...
            <div class="ball-group">
              <div class="group-title">蓝球（选择1个）</div>
              <div class="ball-list">
                <span
                  v-for="num in 16"
                  :key="`blue-${num}`"
          ...
          >
            ...
            <div class="ticket-content" :ref="el => setTicketRef(el, index)">
              <div class="ticket-balls">
                <div class="red-balls-group">
                  <span
                    v-for="num in ticket.red"
                    :key="`red-${num}`"
                    class="ball red-ball"
                  >
                    {{ String(num).padStart(2, '0') }}
                ...
                <div class="blue-balls-group">
                  <span class="ball blue-ball">
                    {{ String(ticket.blue).padStart(2, '0') }}
                  </span>
                </div>
              </div>
            </div>
          ...
          >
            ...
            <div class="preview-ticket-balls">
              <span
                v-for="num in ticket.red"
                :key="`red-${num}`"
                class="preview-ball preview-red-ball"
              >
                {{ String(num).padStart(2, '0') }}
              </span>
              <span class="preview-separator">+</span>
              <span class="preview-ball preview-blue-ball">
                {{ String(ticket.blue).padStart(2, '0') }}
              </span>
            </div>
          </div>
...
const canAddManual = computed(() => {
  return selectedRed.value.length === 6 && selectedBlue.value !== null
})
...
// 切换红球选择
const toggleRedBall = (num) => {
  const index = selectedRed.value.indexOf(num)
  if (index > -1) {
    selectedRed.value.splice(index, 1)
  } else {
    if (selectedRed.value.length < 6) {
      selectedRed.value.push(num)
      selectedRed.value.sort((a, b) => a - b)
    } else {
      ElMessage.warning('最多只能选择6个红球')
    }
  }
}
...
const addManualTicket = () => {
  if (!canAddManual.value) {
    ElMessage.warning('请选择6个红球和1个蓝球')
    return
  }

  tickets.value.push({
    red: [...selectedRed.value],
    blue: selectedBlue.value
  })
...
const downloadPreview = async () => {
  ...
  try {
    ...
    link.download = `双色球推荐_${new Date().getTime()}.png`
    link.href = url
    link.click()
...
.blue-ball-item:hover {
  border-color: #409eff;
  background: #ecf5ff;
}

.blue-ball-item.selected {
